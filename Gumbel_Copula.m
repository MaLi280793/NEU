...
Parameter=['RV_Parameter.xls'];%Randverteilung Parameter der Benchamrk
...
for u=1:length(Dim)
    ...
    for r=1:length(Size)
        ...
        % schätzt Copula Parameter für Gumbel
        families1={'G'};
        [HACObjectC, fitLogC] = HACopulafit(RV_C, families1);
        GumbelR1=zeros(Size(1,r),D/5);
        GumbelR2=zeros(Size(1,r),D/5);
        GumbelR3=zeros(Size(1,r),D/5);
        GumbelR4=zeros(Size(1,r),D/5);
        GumbelR5=zeros(Size(1,r),D/5);
        ...
        for l=1:length(alpha)
            ...
            for i=1:N
                ...
                %Simuliert die Gumbel Copula mit geschätztem Parameter
                GumbelCopula = HACopularnd(HACObjectC, Size(1,r));
                ...
                Matrix_BC = Matrix(1:idx,:); % Matrix für den BC-VaR
                Matrix_WC = Matrix(idx+1:end,:);   % Matrix für den WC-VaR

                %Komontonic VaR
                VaR_Como(i,:)=sum(Matrix(idx+1,:));
                % summiert zuerst die Zeilen und sortiert anschließend VaR(X1+...Xn)
                Ma_sum=sort(sum(Matrix,2));


                %VaR ES/WC
                [VaR_BC(i,:)] = Rearrangement_Algorithmus_VaR_max(Matrix_BC, eps); % berechnet BC-VaR
                [VaR_WC(i,:)]  = Rearrangment_Algorithmus_VaR_min(Matrix_WC, eps); % berechnet WC-VaR

                %ES BC
                [ES_BC(i,:)] =  Rearrangement_Algorithmus_ES_max(Matrix, eps, Size(1,r),alpha_RM);    % berechnet BC-ES
                %ES WC
                Zeilensumme=sum(Matrix,2);% ist schon sortiert (überprüfen)

                ES_WC(i,:)=sum(Zeilensumme(floor((Size(1,r)*alpha_RM))+1:(Size(1,r))))/Size(1,r)/(1-alpha_RM); % berechnet WC-ES
        ...
        Delta_Quo_max= RM_Gumbel(:,4)./VaR_Comontonic(:,2); %passt für alle alpha
        ...
        % Worst Case
        Diff_BC= RM_Gumbel(:,3)-RM_Gumbel(:,1);
        Diff_WC = RM_Gumbel(:,4)-RM_Gumbel(:,2);

        %KOnvergenz sachen
        Theo391_3=Delta_WC.*(VaR_Comontonic(:,2)./RM_Gumbel(:,4));% passt besser
        Theo391=RM_Gumbel(:,2)./RM_Gumbel(:,4);% passt
        ...
        DATEN_C=[RM_Gumbel Spread_VaR Spread_ES  NaN(6, 1); NaN(1, 7)];

        % Speichert die Daten für alle Dimensionen
        %Haupttabelle
        ErgebnisMatrix_Gumbel(u*length(DATEN_C(:,1))-(length(DATEN_C(:,1))-1):u*length(DATEN_C(:,1)),r*length(DATEN_C(1,:))-(length(DATEN_C(1,:))-1):r*length(DATEN_C(1,:)))=[DATEN_C];
    ...
    %speichert die Parameter der Gumbel Copula für 25000 Beobachtungen
    CopulaParameter(u,1)=HACObjectC.Parameter;
    toc
end
%% Speichert Daten
xlswrite([Hauptpfad RohDatenGumbel], CopulaParameter, 'Theta','B1');
xlswrite([Hauptpfad RohDatenGumbel], rowname1, 'Theta','A1');
xlswrite([Hauptpfad RohDatenGumbel], columnNames, 'Tabellen', 'A1');
xlswrite([Hauptpfad RohDatenGumbel], rowname2, 'Tabellen', 'A2');
xlswrite([Hauptpfad RohDatenGumbel], ErgebnisMatrix_Gumbel, 'Tabellen', 'B2');


%Kennzahlen
xlswrite([Hauptpfad RohDatenGumbel], Kennzahlen_all, 'Kennzahlen', 'B2');
xlswrite([Hauptpfad RohDatenGumbel], Zeilenvektor_Kennzahlen, 'Kennzahlen', 'A1');
xlswrite([Hauptpfad RohDatenGumbel], rowname2, 'Kennzahlen', 'A2');


% Konvergenz
xlswrite([Hauptpfad RohDatenGumbel], Konvergenz_all, 'Konvergenz', 'B2');
xlswrite([Hauptpfad RohDatenGumbel], Row_konv, 'Konvergenz', 'A1');
xlswrite([Hauptpfad RohDatenGumbel], Colu_Konv, 'Konvergenz', 'A2');
