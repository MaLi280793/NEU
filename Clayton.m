clear all
% Benchamrk muss zuerst simuliert werden
...
Parameter=['RV_Parameter.xls'];%Parameter der Benchamrk
...
Zeilenvektor_Kennzahlen={'Dim05' 'SpreadQuot' '' 'Theo391(VaR/ES)' '' '1' 'WC_Delta' '' 'Theo391_3' 'Delta_Quo_max' ''...
    ...
    'SpreadQuot' '' 'Theo391(VaR/ES)' '' '1' 'WC_Delta' '' 'Theo391_3' 'Delta_Quo_max' ''...
    'SpreadQuot' '' 'Theo391(VaR/ES)' '' '1' 'WC_Delta' '' 'Theo391_3' 'Delta_Quo_max' ''};
RohDaten_Clayton={'Rohdaten_Clayton'};
%Haupttabellen


rowname1={'Dim05';'Dim10';'Dim15';'Dim20'};%muss bleiben
Bereich={'B1','B2','B3','B4'};%muss bleiben
%Lädt die Parameter der Randverteilungen der Benchmark
...
for u=1:length(Dim)
    ...
    for r=1:length(Size)
        ...
        % schätzt Copula Parameter für Clayton
        families1={'C'};
        [HACObjectC, fitLogC] = HACopulafit(RV_C, families1);
        ClaytonR1=zeros(Size(1,r),D/5);
        ClaytonR2=zeros(Size(1,r),D/5);
        ClaytonR3=zeros(Size(1,r),D/5);
        ClaytonR4=zeros(Size(1,r),D/5);
        ClaytonR5=zeros(Size(1,r),D/5);
        ...
        for l=1:length(alpha)
            ...
            for i=1:N
                ...
                %Simuliert die Clayton Copula mit geschätztem Parameter
                Clayton_Copula = HACopularnd(HACObjectC, Size(1,r));
                Ma_Clayton=sort(Clayton_Copula);

                % Rücktranformation der Randverteilungen mithilfe der
                % fixierten Randverteilungs Parameter
                for j=1:D/5
                ...
                Clayton_Matrix_BC = Clayton_Matrix(1:idx,:); % Matrix für den BC-VaR
                Clayton_Matrix_WC = Clayton_Matrix(idx+1:end,:);   % Matrix für den WC-VaR

                %Komontonic VaR
                VaR_Como(i,:)=sum(Clayton_Matrix(idx+1,:));
                % summiert zuerst die Zeilen und sortiert anschließend VaR(X1+...Xn)
                Ma_sum=sort(sum(Clayton_Matrix,2));



                %VaR ES/WC
                [VaR_BC(i,:)] = Rearrangement_Algorithmus_VaR_max(Clayton_Matrix_BC, eps); % berechnet BC-VaR
                [VaR_WC(i,:)]  = Rearrangment_Algorithmus_VaR_min(Clayton_Matrix_WC, eps); % berechnet WC-VaR

                %ES BC
                [ES_BC(i,:)] =  Rearrangement_Algorithmus_ES_max(Clayton_Matrix, eps, Size(1,r),alpha_RM);    % berechnet BC-ES
                %ES WC
                Zeilensumme_Clayton=sum(Clayton_Matrix,2);% ist schon sortiert (überprüfen)

                ES_WC(i,:)=sum(Zeilensumme_Clayton(floor((Size(1,r)*alpha_RM))+1:(Size(1,r))))/Size(1,r)/(1-alpha_RM); % berechnet WC-ES
        ...
        Delta_Quo_max= RM_Clayton(:,4)./VaR_Comontonic(:,2); %passt für alle alpha
        ...
        % Worst Case
        Diff_BC= RM_Clayton(:,3)-RM_Clayton(:,1);
        Diff_WC = RM_Clayton(:,4)-RM_Clayton(:,2);


        %KOnvergenz sachen
        Theo391_3=Delta_WC.*(VaR_Comontonic(:,2)./RM_Clayton(:,4));% passt besser
        Theo391=RM_Clayton(:,2)./RM_Clayton(:,4);% passt


        % Speichert verschiede Variablen in eine Matrix
        ...
        DATEN_C=[RM_Clayton Spread_VaR Spread_ES  NaN(6, 1); NaN(1, 7)];



        % Speichert die Daten für alle Dimensionen
        %Haupttabelle
        ErgebnisMatrix_Clayton(u*length(DATEN_C(:,1))-(length(DATEN_C(:,1))-1):u*length(DATEN_C(:,1)),r*length(DATEN_C(1,:))-(length(DATEN_C(1,:))-1):r*length(DATEN_C(1,:)))=[DATEN_C];
...
%% Speichert Daten
xlswrite([Hauptpfad RohDaten_Clayton], CopulaParameter, 'Theta','B1');
xlswrite([Hauptpfad RohDaten_Clayton], rowname1, 'Theta','A1');
xlswrite([Hauptpfad RohDaten_Clayton], columnNames, 'Tabellen', 'A1');
xlswrite([Hauptpfad RohDaten_Clayton], rowname2, 'Tabellen', 'A2');
xlswrite([Hauptpfad RohDaten_Clayton], ErgebnisMatrix_Clayton, 'Tabellen', 'B2');




%Kennzahlen
xlswrite([Hauptpfad RohDaten_Clayton], Kennzahlen_all, 'Kennzahlen', 'B2');
xlswrite([Hauptpfad RohDaten_Clayton], Zeilenvektor_Kennzahlen, 'Kennzahlen', 'A1');
xlswrite([Hauptpfad RohDaten_Clayton], rowname2, 'Kennzahlen', 'A2');


% Konvergenz
xlswrite([Hauptpfad RohDaten_Clayton], Konvergenz_all, 'Konvergenz', 'B2');
xlswrite([Hauptpfad RohDaten_Clayton], Row_konv, 'Konvergenz', 'A1');
xlswrite([Hauptpfad RohDaten_Clayton], Colu_Konv, 'Konvergenz', 'A2');
